...
def create_course():
    try:
        print("Starting course creation...")
        ...
        print(f"Course info: {subject}, {number_lesson} lessons, {duration_lesson}h each")
        ...
        # Sử dụng thông tin sách đã được xác nhận
        pdf_info = {
            "book": book_name,
        ...
        print(f"Generated new course ID: {new_id}")
        ...
        print(f"Moved PDF to final location: {final_path}")

        # Chia sách - có thể timeout ở đây
        print("Starting book splitting...")
        chiasach_dir = os.path.join(UPLOAD_FOLDER, str(new_id), "chiasach")
        try:
            chiasach(pdf_info['content'], final_path, chiasach_dir)
            print("Book splitting completed successfully")
        except Exception as e:
            print(f"Error in chiasach: {str(e)}")
            # Tiếp tục xử lý dù có lỗi trong chia sách
        
        print("Starting CLO generation...")
        # Generate CLOs với timeout ngắn hơn
...
{pdf_info['content']}
        ...
        try:
            # Generate CLOs với timeout
            initial_clo = chat_CLO(
                conversation_history=[],
                user_input = initial_prompt
            )
            print("CLO generation completed")
        except Exception as e:
            print(f"Error generating CLO: {str(e)}")
            initial_clo = "Lỗi khi tạo CLO. Vui lòng thử lại sau."
        ...
        print(f"Course created successfully with ID: {new_id}")
