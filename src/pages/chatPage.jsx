...
import botImage from '../assets/images/bot_image.jpg';
...
const Message = ({ message, selectedTitle, onLike, isProcessingSessions, messages }) => {
  ...
  console.log("hasLikedMessage:", hasLikedMessage);
  console.log("messages with liked status:", messages.map(m => ({ 
    role: m.role, 
    liked: m.liked, 
    content: m.content?.substring(0, 50) + "..." 
  })));
  console.log("Should show CLO button:", selectedTitle === "CLOs" && role === "assistant");
  console.log("Button condition:", !hasLikedMessage && !isLoading);
  console.log("===================");
  ...
  return (
    <div className={`message ${role === "assistant" ? "received" : "sent"}`}>
      ...
      <div className="message-content">
        ...
        {/* Like Button - Simple Icon Only */}
        {(selectedTitle === "CLOs" || selectedTitle === "Course Syllabus") && role === "assistant" && (
          <div className="like-button-container">
            {liked ? (
              <button className="like-button liked" disabled>
                <i className="fa-solid fa-thumbs-up"></i>
              </button>
            ) : (
              !hasLikedMessage && !isLoading && (
                <button
                  className="like-button"
                  onClick={() => onLike(content, selectedTitle === "CLOs" ? "CLOs" : "Syllabus")}
  ...
  const fetchCourseData = async () => {
    try {
      ...
      if (currentCourse) {
        ...
        console.log("Full course:", currentCourse);
        console.log("CLO messages:", currentCourse.message_CLOs);
        console.log("Each CLO message liked status:", 
          currentCourse.message_CLOs?.map(m => ({ 
            role: m.role, 
            liked: m.liked,
